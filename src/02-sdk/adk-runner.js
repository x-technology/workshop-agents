...
import { fallbackClassification, normalizeClassification } from './fallback.js';
import { safeJsonParse } from './json.js';

export function formatEmailForAgent(email) {
  return [`From: ${email.from ?? 'unknown'}`, `Subject: ${email.subject ?? ''}`, `Body: ${email.body ?? ''}`].join(
    '\n',
  );
}
...
export async function runClassificationAgent({ email, model }) {
  return runEmailAgent({
    ...
    promptLines: ['EMAIL_TO_CLASSIFY', formatEmailForAgent(email)],
  });
}
